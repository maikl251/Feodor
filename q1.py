...
class QTextNetwork:
    ...
    def train(self, epochs=5, batch_size=32, use_early_stopping=True, loss_threshold=0.01):
        ...
        # ОБУЧЕНИЕ НА ПСЕВДОПАРАХ ИЗ VAE
        for pair in self.query_answer_pairs:
            pseudo_question = pair['question']
            pseudo_answer = pair['answer']
            
            # Генерация нескольких кандидатов с помощью трансформера
            for candidate_num in range(3):
                # Склеивание псевдовопроса и псевдоответа как семантического ядра
                semantic_core = pseudo_question.split() + pseudo_answer.split()
                candidate_text, _ = self.generate_text_from_seed(
                    semantic_core, 
                    max_length=15, 
                    temperature=0.7 + candidate_num * 0.1
                )
                
                # ВЫЧИСЛЕНИЕ УПРОЩЕННОГО ВОЗНАГРАЖДЕНИЯ (БЕЗ CO-OCCURRENCE)
                ...
                # ФИЛЬТРАЦИЯ: пропускаем кандидатов с низким reward
                if reward < 0.2:
                    continue
                ...
                for i in range(1, min(len(words), 8)):
                    ...
                    if next_word in self.tokenizer.word_index:
                        ...
                        if word_idx < self.vocab_size:
                            # Основное слово получает высокую вероятность на основе reward
                            main_prob = 0.1 + (reward * 0.7)  # Диапазон: 0.1-0.8
                            target_vector[word_idx] = main_prob
                    
                    # Распределяем оставшуюся вероятность
                    remaining_prob = 1.0 - np.sum(target_vector)
                    if remaining_prob > 0:
                        # Равномерное распределение по всем словам
    ...
    def generate_enhanced_response(self, query, max_length=20, temperature=0.8):
        """УЛУЧШЕННАЯ генерация БЕЗ CO-OCCURRENCE"""
        
        # Поиск семантически близкой пары
        best_pair, similarity_score = self.find_similar_question(query)
        
        candidates = []
        
        if best_pair and similarity_score > 0.5:
            # Использование найденной пары как семантического ядра
            semantic_core = best_pair['question'].split() + best_pair['answer'].split()
            
            # Генерация нескольких кандидатов
            for i in range(3):
                candidate, confidence = self.generate_text_from_seed(
                    semantic_core, 
                ...
                candidates.append((candidate, reward, confidence))
        else:
            # Генерация с нуля
            for i in range(3):
                candidate, confidence = self.generate_text_from_seed(
                ...
                reward = self.get_enhanced_chain_reward(query, "", candidate)
                candidates.append((candidate, reward, confidence))
        
        # Выбор лучшего кандидата
        if candidates:
            best_candidate = max(candidates, key=lambda x: x[1])  # Максимизация вознаграждения
            return best_candidate[0], best_candidate[1]  # Текст и уверенность
        
        return query, 0.5
    
    def generate_text(self, query, max_length=20, temperature=1.0):
        # Простая обертка
        return self.generate_enhanced_response(query, max_length, temperature)
