...
class KLAnealing(tf.keras.callbacks.Callback):
    def __init__(self, beta_start=0.0, beta_end=1.0, steps=2000):#10000->2000
        super().__init__()
...
class ImprovedVAELossLayer(Layer):
    ...
    def call(self, inputs):
        ...
        # Reconstruction loss с маской padding
        mask = tf.cast(tf.not_equal(x, 0), tf.float32)          # 1 — реальные токены, 0 — padding
    
        xent = tf.keras.losses.sparse_categorical_crossentropy(
            x, x_decoded_mean, from_logits=True
        )
        xent_masked = xent * mask
    
        # Среднее только по реальным токенам (самый правильный способ)
        xent_loss = tf.reduce_sum(xent_masked) / (tf.reduce_sum(mask) + 1e-12)
...
class TextVAE:
    def __init__(self):
        ...
        self.vocab_size = 50000 # увеличил с 10000
        self.latent_dim = 256
    ...
    def clean_text(self, text):
        """
        Очистка текста: оставляет только кириллические символы и пробелы
        Удаляет цифры, латиницу, пунктуацию, специальные символы
        """
        # Оставляем только кириллические буквы и пробелы
        ...
        for file_path in glob.glob(os.path.join(texts_dir, "**", "*.txt"), recursive=True):
            ...
            if file_key not in self.processed_files:
                try:
                    ...
                    # Очистка текста перед лемматизацией
                    cleaned_text = self.clean_text(text_content)
                    
                    # без Лемматизации
                    words = re.findall(r'\b\w+\b', cleaned_text)
                    
                    lemmatized_text = ' '.join(word for word in words if word.strip())
    ...
    def build_model(self):
        ...
        x = Embedding(self.vocab_size, 256, mask_zero=False)(inputs)
        ...
        # ИЗМЕНЕНИЕ 1: добавить квадратичные признаки перед Dense слоями
        x_squared = tf.keras.layers.Lambda(lambda t: tf.concat([t, tf.square(t)], axis=-1))(x)
    
        z_mean = Dense(self.latent_dim)(x_squared)  
        z_log_var = Dense(self.latent_dim)(x_squared)  
        ...
        # ИЗМЕНЕНИЕ 2: добавить квадратичные признаки в декодере
        latent_squared = tf.keras.layers.Lambda(lambda t: tf.concat([t, tf.square(t)], axis=-1))(latent_inputs)
    
        x = Dense(self.max_sequence_length * 64)(latent_squared)  
        x = Reshape((self.max_sequence_length, 64))(x)
    
        positions = tf.range(self.max_sequence_length)[tf.newaxis, :]
        pos_embedding = tf.keras.layers.Embedding(self.max_sequence_length, 64)(positions)
        x = x + pos_embedding
    ...
    def load_model(self, model_dir):
        try:
            ...
            logging.info(f"Модель VAE не загружена, будет создана новая: {e}")
            return False
    ...
    def prepare_training_data(self, texts):
        ...
        # Добавить sliding windows (если текстов мало):
        if len(filtered_texts) < 100:  # Только если корпус маленький
            additional_sequences = []
            for text in filtered_texts:
                words = text.split()
                if len(words) > 20:  # Только для достаточно длинных текстов
                    for i in range(0, len(words) - 200, 170): # Шаг 200 слов для окна перекрытия 30
                        window = ' '.join(words[i:i + 20])
                        # Токенизируем окно
                        seq = self.tokenizer.texts_to_sequences([window])[0]
                        padded = pad_sequences([seq], maxlen=self.max_sequence_length, padding='post')[0]
                        additional_sequences.append(padded)
    
            if additional_sequences:
                augmented_sequences.extend(additional_sequences)
                logging.info(f"[vae] Добавлено {len(additional_sequences)} sliding windows")
    ...
    def train_model(self, epochs=10, batch_size=32, new_texts=None, use_early_stopping=True, loss_threshold=0.01):
        ...
        max_epochs = min(epochs, 1000) # Здесь epochs - 1000
    ...
    def generate_queries_from_centroids(self, centroids, num_queries_per_topic, temperature):
        ...
        try:
            for theme, centroid in centroids.items():
                ...
                centroid_variation = random.uniform(0.3, 1.5)  # Коэффициент вариации центроида
            
            
                for i in range(num_queries_per_topic):
                    # Увеличиваем случайность для каждой генерации
                    current_temperature = temperature * (1.0 + 0.50 * random.random())  # ±20% к температуре
                    ...
                    # Использовать с температурой:
                    temperature = random.uniform(0.8, 1.2)  # было 0.7
                    for t in range(self.max_sequence_length):
                        logits = sequence[t] / temperature  # <-- добавить температуру
                        ...
                        for attempt in range(10):  # Максимум 10 попыток
                            if len(used_words) < self.vocab_size * 0.2:  # Если использовано меньше 10% слов
                                word_idx = top_k_sampling(probs, k=min(30, len(probs)//2))
                            else:
